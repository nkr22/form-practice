...
const Scriptures = (function () {
    ...
    ajax = function (url, successCallback, failureCallback, skipJsonParse) {
        ...
        // .then(async (response) => {
        //     if(response.ok){
        //         const data=await (skipJsonParse)?response.text():response.json());
        //         data.then(data=> {
        ...
        volumes.forEach(function (volume) {
            if (volumeId === undefined || volumeId === volume.id) {
                gridContent += htmlDiv({
                    ...
                    content:
                        htmlAnchor(volume) +
                        htmlElement(TAG_HEADER5, volume.fullName),
                });
